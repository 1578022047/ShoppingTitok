...
public class HttpUtil {
    ...
    //加入购物车
    public static void addBuyCarHttp(String address,String userId,String videoId,okhttp3.Callback callback){
        OkHttpClient client = new OkHttpClient();
        FormBody.Builder requestBody = new FormBody.Builder();
        requestBody.add("videoId",videoId).add("userId",userId);
        Request request  =new Request.Builder().url(address)
                .post(requestBody.build())
                .build();
        client.newCall(request).enqueue(callback);
    ...
    //获取用户购物车
    public static void getUserBuyCarHttp(String address,String userId,okhttp3.Callback callback){
        OkHttpClient client = new OkHttpClient();
        FormBody.Builder requestBody = new FormBody.Builder();
        requestBody.add("userId",userId);
        Request request  =new Request.Builder().url(address)
